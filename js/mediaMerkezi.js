...
const mediaMerkezi = document.querySelectorAll('.media-merkezi')
let togle=false;

mediaMerkezi.forEach(d=>{
...
const muharibeSehidleri = document.querySelector('.muharibe-sehidleri');
const mediaConteiner = document.querySelectorAll('.media-conteiner section');
const aboutMuharibeSehidleri = document.querySelector('#muharibe-sehidleri');
const boxs = document.querySelectorAll('.mySwiper .box');
const input = document.querySelector('.input');

muharibeSehidleri.addEventListener('click', ()=>{
    mediaConteiner.forEach(media =>{
        media.classList.remove('active')
    })
    aboutMuharibeSehidleri.classList.add('active')
})

input.addEventListener('keyup',(event) =>{
    const value = event.target.value.toLowerCase();
    boxs.forEach(box =>{
        box.querySelector('.head h4').textContent.toLocaleLowerCase().startsWith(value),
        box.querySelector('.metn p').textContent.toLocaleLowerCase().startsWith(value)
